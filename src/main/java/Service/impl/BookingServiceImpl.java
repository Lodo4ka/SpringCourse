...
public class BookingServiceImpl implements BookingService
{
    ...
    public BookingServiceImpl(List<Ticket> tickets) {
        this.tickets = tickets;
    }

    @Override
    public long getTicketsPrice(Event event, LocalDateTime dateTime, User user, int seats) {
        return tickets.stream().filter(ticket -> ticket.getEvent().equals(event) && ticket.getDateTime().equals(dateTime)
                && ticket.getUser().equals(user) && ticket.getSeat() == seats).mapToLong(ticket -> event.getBasePrice()).sum();
    }
